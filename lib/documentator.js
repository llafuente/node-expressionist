(function () {
    ...
    */
    ...
    module.exports = {
        ...
        URIClientFunction: function(uri) {
            var doc = this.doc,
                pdoc = [],
                args = [],
                idx;

            doc.push("<h3>Client</h3>");

            if (uri.params) {
                for (idx in uri.params) {
                    args.push((uri.params[idx].cast || "mixed") + " " +idx);
                }
            }
            args.push("Object get");
            args.push("Object post");
            args.push("Function callback");

            doc.push('<pre class="parameters">' + uri.group + "." + uri.clientFunction + "(" + args.join(", ") + ")");
            doc.push("</pre>");
        },

        loopUris: function (expresionist, uris, groups) {
            //console.log(util.inspect(uris, {depth: 7, colors: true}));
                ...
                for (i = 0; i < uri_list.length; ++i) {
                    ...
                    for (j = 0; j < order.length; ++j) {
                        ...
                        if (uri_data !== undefined) {
                            ...
                            this.URIClientFunction(uri_data);
                        }
                    }
            ...
            module.exports.doc[8] = '<div id="index"><ul><li>' + index.join("</li><li>") + "</li></ul></div>";
            module.exports.doc.push("</div>");// close content
            module.exports.doc.push("</body>");
            module.exports.doc.push("</html>");

            return module.exports.doc.join("\n");
        },
        // file it's optional, allways return
        save: function (expresionist, file, groups) {
            var doc = this.loopUris(expresionist, expresionist.uris, groups),
                base_dir = path.dirname(expresionist.rootDir + path.sep + file),
