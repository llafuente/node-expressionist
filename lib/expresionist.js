...
expresionist.prototype.requestHook = function (name, callback, doc) {
    ...
    if (callback.length !== 4) {
        throw new Error("requestHook callback must have 4 parameters");
    }
...
expresionist.prototype.responseHook = function (name, callback, doc) {
    ...
    if (callback.length !== 5) {
        throw new Error("responseHook callback must have 5 parameters");
    }
...
expresionist.prototype.uri = function (uri, method, options) {
    ...
    if (handler.length !== 4) {
        throw new Error(method + ":" + uri + " handler must have 4 parameters");
    }
...
expresionist.prototype.validateInput = function (validation, to_check, res) {
    ...
    for (name in validation) {
        ...
        console.log("validate ==> ", name, current_validation);
        console.log("against ==> ", to_check[name]);
        //validators[]
