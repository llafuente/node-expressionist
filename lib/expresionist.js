(function () {
    ...
    object.each(validator, function (v, k) {
        k = k.toLowerCase();
        if (k.indexOf("is") !== 0) {
            k = "is" + k;
        }
    ...
    validators.isoptional = function() {
        return true;
    };
    ...
    expresionist.prototype.validateObject = function (validation, to_check, options, res) {
        ...
        for (name in validation) {
            ...
            this.verbose("validate ", {name: name, properties: param_properties, value: current_value});
    ...
    function handle_iteration(req, res, express_next, self) {
        ...
        case "checkRequest":
            ...
            search.forEach(function (target) {
                ...
                if (validation !== null) {
                    this.expresionist.verbose("validation: ", validation, "\nagainst: ", to_check);
        ...
        //build the response!
        this.expresionist.debug("response", res.content);
        ...
        this.expresionist.debug("result", ret);
        res.send(ret);
        res.end();
