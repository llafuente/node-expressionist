(function () {
    ...
    function uri_parameters(expressionist, type, bag, documentation, path) {
        if (!bag) {
            return null;
        }
        path = path || "";

        var app = expressionist.app,
            idx,
            doc,
            constraints,
            constraint_key;
        ...
        for (idx in bag) {
            ...
            doc = bag[idx].doc || documentation.params[idx] || documentation.params[path + idx] || null;

            if (!bag[idx].doc) {
                expressionist.warn("missing parameter [" + idx + "] documentation");
            } else {
                bag[idx].doc = doc.replace(/[\s]+$/g, '');
            }


            if ("object" === bag[idx].cast) {
                bag[idx].object = uri_parameters(expressionist, type, bag[idx].object, documentation, path + idx + ".");
            }
            if (bag[idx].constraints) {
                if (object.empty(bag[idx].constraints)) {
                    bag[idx].constraints = null;
                } else {
                    constraints = bag[idx].constraints;

                    for(constraint_key in constraints) {
                        if (constraints[constraint_key] === null) {
                            expressionist.warn("missing constraint text [" + idx + "/" + constraint_key + "] documentation");
                            constraints[constraint_key] = ["constraint [" + constraint_key + "] fail"];
                        } else if ("string" !== typeof (constraints[constraint_key][constraints[constraint_key].length -1])) {
                            expressionist.warn("missing constraint text [" + idx + "/" + constraint_key + "] documentation");
                            constraints[constraint_key].push("constraint [" + constraint_key + "] fail");
                        }
                    }
                }
            }

            if (!bag[idx].constraints && bag[idx].cast) {
                bag[idx].constraints = {};
                bag[idx].constraints[bag[idx].cast] = [idx + "-cannot-be-casted"];

            }

            /* express-params
            if (type === "params") {
                if (bag[idx].cast === "integer") {
                    app.param(idx, /^\d+$/);
                }
            }
            */
    ...
    expresionist.prototype.uri = function (uri, method, options) {
        ...
        params = uri_parameters(this, "params", options.params, this.documentation);
        query = uri_parameters(this, "query", options.get, this.documentation);
        body = uri_parameters(this, "body", options.post, this.documentation);
        response = uri_parameters(this, "response", options.response || {}, this.documentation);
    ...
    expresionist.prototype.validateObject = function (validation, to_check, options, res) {
        ...
        for (name in validation) {
            ...
            if (constraints) {
                ...
                try {
                    errors = utilitario.validate(current_value, constraints)

                    if (errors !== false) {
                        errors.forEach(function(v) {
                            res.addError(options.status, options.message, 1002, v);
                        })
                    }
                } catch(e) {
                    res.addError(options.status, options.message, 1001, e.message);
                }

                // cast should be only valid to input
                if (options.cast && param_properties.cast !== undefined) {
                    cast_fn = __cast[param_properties.cast];
            ...
            } else if ("object" === param_properties.cast) {
    ...
    function handle_iteration(req, res, express_next, self) {
        this.expresionist.verbose("# status", this.status, this.itr);
        if (res.hasErrors && res.hasErrors()) {
            this.expresionist.verbose("# errors ?", res.content.errors);
        }
        ...
        case "checkRequest":
            this.expresionist.verbose(this.route);
            //this.expresionist.verbose(req);
            // params, get, post
            ...
            search.forEach(function (target) {
                ...
                if (validation !== null) {
                    this.expresionist.verbose("validate [", target, "]: ", validation, "\nagainst: ", to_check);
        ...
        case "globalRequestHooks":
            if (!res.hasErrors() || this.route.requestHooksPolicy === "CONTINUE_ON_ERROR") {
                hooks = this.expresionist.hooks.gRequest;
                if (hooks && hooks.length > this.itr) {
                    current_hook_name = hooks[this.itr];
                    this.expresionist.verbose("exec: global request hook", current_hook_name);
        ...
        case "requestHooks":
            if (!res.hasErrors() || this.route.requestHooksPolicy === "CONTINUE_ON_ERROR") {
                ...
                if (hooks && hooks.length > this.itr) {
                    current_hook_name = hooks[this.itr];
                    this.expresionist.verbose("exec: request hook", current_hook_name);
                    fn = this.expresionist.hooks.request[current_hook_name];
        ...
        case "globalResponseHooks":
            if (!res.hasErrors()) {
                hooks = this.expresionist.hooks.gResponse;
                if (hooks && hooks.length > this.itr) {
                    current_hook_name = hooks[this.itr];
                    console.log("connection ", current_hook_name);
                    this.expresionist.verbose("exec: global response hook", current_hook_name);
        ...
        case "responseHooks":
            if (!res.hasErrors()) {
                ...
                if (hooks && hooks.length > this.itr) {
                    current_hook_name = hooks[this.itr];
                    this.expresionist.verbose("exec: response hook", current_hook_name);
                    fn = this.expresionist.hooks.response[current_hook_name];
        ...
        if (res.content.errors.length > 0) {
            ...
            this.expresionist.info("error-response", ret);
        } else {
            ...
            this.expresionist.info("success-response", ret);
        }
    ...
    expresionist.prototype.call = function (uri, method, req, callback, res) {
        ...
        this.log("#params", inspect(req.params, {depth:1}));
        this.log("#query", inspect(req.query, {depth:1}));
        this.log("#body", inspect(req.body, {depth:1}));
        this.log("#method", inspect(req.method, {depth:1}));
    ...
    expresionist.prototype.logLevel = 5;
