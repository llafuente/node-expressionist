    ...
    expresionist.prototype.nodeClient = function (base_url, post_process) {
        ...
        object.each(this.uris, function (list, method) {
            object.each(list, function (uri) {
                ...
                //uri.clientFunction
                var fn_name = uri.clientFunction;
                if (fn_name.indexOf(uri.group) !== 0) {
                    self.warning("(expressionist) It's recommended to start functionName with group name, found:", fn_name);
                } else {
                    fn_name = fn_name.substring(uri.group.length + 1);
                }
                fn_name = utilitario.transform.toCamelCase(fn_name);

                client[uri.group][fn_name] = function () {
