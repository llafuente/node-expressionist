(function () {
    ...
    expresionist.prototype.loadYML = function (yml_files, group, callback) {
        ...
        if ("development" === enviroment) {
            this.debug("load YML(s) ", yml_files, group);
        }
        ...
        var data = '',
            ...
            load_next = function () {
                if (yml_files.length === yml_file_id) {
                    ...
                    if ("development" === enviroment) {
                        this.debug("reading YML: ", yml_file);
                    }
    ...
    expresionist.prototype.uri = function (uri, method, options) {
        if ("development" === enviroment) {
            this.debug("define uri", method, ":", uri);
        }
    ...
    expresionist.prototype.validateObject = function (validation, to_check, options, res) {
        ...
        for (name in validation) {
            ...
            this.debug("validate ", {name: name, properties: param_properties, value: current_value});
    ...
    function handle_iteration(req, res, express_next, self) {
        ...
        case "requestHooks":
            if (!res.hasErrors() || this.route.requestHooksPolicy === "CONTINUE_ON_ERROR") {
                ...
                if (hooks && hooks.length > this.itr) {
                    ...
                    this.expresionist.verbose("exec", current_hook_name);
        ...
        case "responseHooks":
            if (!res.hasErrors()) {
                ...
                if (hooks && hooks.length > this.itr) {
                    current_hook_name = hooks[this.itr];
                    this.verbose("exec", current_hook_name);
